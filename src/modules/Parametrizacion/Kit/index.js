...
import KitCreador from './KitCreador';
import {
  onGetColeccion,
  onDelete,
...
const Kit = (props) => {
  ...
  const onOpenViewKit = (id) => {
    setKitSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteKit = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar El Kit?',
  ...
  const onGoComponentes = (cliente_id, id) => {
    history.push('kits-componentes/'+cliente_id+'/'+id);
  }
                          ...
                          <TableCell align='center' className={classes.cell}>
                            <Tooltip title={'Componentes'}>
                              <Box style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                                <ListAlt 
                                  onClick={() => onGoComponentes(row.cliente_id, row.id)}
