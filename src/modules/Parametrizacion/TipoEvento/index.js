...
import TipoEventoCreador from './TipoEventoCreador';
import {
  onGetColeccion,
  onDelete,
...
import { CLASES_EVENTOS, DATO_BOOLEAN, ESTADOS_SELLOS } from 'shared/constants/ListaValores';
...
const cells = [
  ...
  },
  {
    ...
    typeHead: 'string',
    label: 'Estado Sello',
    value: (value) => ESTADOS_SELLOS.map((estado) => (estado.id === value ? estado.nombre : '')),
    align: 'left',
    mostrarInicio: true,
  },
  {
    id: 'indicativo_evento_manual',
    typeHead: 'string',
    label: 'Evento Manual',
  ...
  {
    id: 'indicativo_clase_evento',
    typeHead: 'string',
    label: 'Clase Evento',
    value: (value) => CLASES_EVENTOS.map((dato) => (dato.id === value ? dato.nombre : '')),
    align: 'left',
    mostrarInicio: false,
  },
  {
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip
                  title='Crear Tipo Evento'
                  onClick={onOpenAddTipoEvento}>
                  <IconButton
...
const TipoEvento = (props) => {
  ...
  const onOpenEditTipoEvento = (id) => {
    setTipoEventoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewTipoEvento = (id) => {
    setTipoEventoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteTipoEvento = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar el Tipo de Evento?',
  ...
  const onOpenAddTipoEvento = () => {
    setTipoEventoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
      ...
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddTipoEvento={onOpenAddTipoEvento}
