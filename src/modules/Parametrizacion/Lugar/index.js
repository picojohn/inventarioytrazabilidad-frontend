...
import LugarCreador from './LugarCreador';
import {
   onGetHeaders,
   onGetColeccion, 
   onDelete,
...
import { CLIENTES_LUGARES } from 'shared/constants/SubmenuOptions';
...
const cells = [
   ...
   {
      id: 'indicativo_lugar_remision',
      typeHead: 'string',
      label: 'Lugar remisión',
   ...
   {
      id: 'indicativo_lugar_instalacion',
      typeHead: 'string',
   ...
   {
      id: 'indicativo_lugar_inspeccion',
      typeHead: 'string',
               ...
               <>
                  <Box className = { classes.titleTop }>
                     ...
                     <Box className = { classes.horizontalBottoms }>
                        ...
                        {permisos.indexOf('CrearLug') >= 0 && (
                           <Tooltip title = 'Crear Lugar' onClick = { onOpenAddLugar }>
                              <IconButton
...
EnhancedTableToolbar.propTypes = {
   numSelected: PropTypes.number.isRequired,
   onOpenAddLugar: PropTypes.func.isRequired,
...
const Lugares = (props) => {
   ...
   const headers = useSelector(({ lugarReducer }) => lugarReducer.headers);
         ...
         user.permisos.forEach((modulo) => {
            modulo.opciones.forEach((opcion) => {
               if (opcion.url === '/clientes') {
                  setTitulo(`${opcion.nombre} - Lugares`);
                  const permisoAux = [];
                  opcion.permisos.forEach((permiso) => {
                     const event = (perm) => perm.nombre === permiso.titulo;
                     const aux = CLIENTES_LUGARES.some(event);
   ...
   const onOpenViewLugar = (id) => {
      setLugarSeleccionado(id);
      setAccion('ver');
      setShowForm(true);
   };

   const onDeleteLugar = (id) => {
      Swal.fire({
         title: 'Confirmar',
         text: '¿ Seguro que desea eliminar el Lugar ?',
                                       ...
                                       return (
                                          <TableRow
                                             ...
                                             <TableCell 
                                                ...
                                                {
                                                   permisos.indexOf('ModificarLug') >= 0 && 
                                                   (
                                                      <Tooltip title = { <IntlMessages id = 'boton.editar' />}>
                                                         <EditIcon
                                                            onClick = { () => onOpenEditLugar(row.id) }
                                                ...
                                                {
                                                   permisos.indexOf('ListarLug') >= 0 && 
                                                   (
                                                      <Tooltip title = { <IntlMessages id = 'boton.ver' />}>
                                                         <VisibilityIcon
                                                            onClick = { () => onOpenViewLugar(row.id) }
                                                ...
                                                {
                                                   permisos.indexOf('EliminarLug') >= 0 && 
                                                   (
                                                      <Tooltip
                                                         title = { <IntlMessages id = 'boton.eliminar' />}>
                                                         <DeleteIcon
                                                            onClick = { () => onDeleteLugar(row.id) }
