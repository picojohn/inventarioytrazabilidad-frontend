...
import ClienteAlertaCreador from './ClienteAlertaCreador';
import {
   onGetHeaders,
   onGetColeccion, 
   onDelete,
...
import { CLIENTES_ALERTAS } from 'shared/constants/SubmenuOptions';
               ...
               <>
                  <Box className = { classes.titleTop }>
                     ...
                     <Box className = { classes.horizontalBottoms }>
                        ...
                        {permisos.indexOf('CrearAle') >= 0 && (
                           <Tooltip title = 'Crear Alerta' onClick = { onOpenAddClienteAlerta }>
                              <IconButton
...
EnhancedTableToolbar.propTypes = {
   numSelected: PropTypes.number.isRequired,
   onOpenAddClienteAlerta: PropTypes.func.isRequired,
...
const ClienteAlertas = (props) => {
   ...
   const [clienteAlertaSeleccionado, setClienteAlertaSeleccionado] = useState(0);
   const { rows, desde, hasta, ultima_pagina, total } = useSelector(
      ({ clienteAlertaReducer }) => clienteAlertaReducer,
   );

   const { message, error, messageType } = useSelector(({ common }) => common);
   const headers = useSelector(({ clienteAlertaReducer }) => clienteAlertaReducer.ligera);
         ...
         user.permisos.forEach((modulo) => {
            modulo.opciones.forEach((opcion) => {
               if (opcion.url === '/clientes') {
                  setTitulo(`${opcion.nombre} - Alertas`);
                  const permisoAux = [];
                  opcion.permisos.forEach((permiso) => {
                     const event = (perm) => perm.nombre === permiso.titulo;
                     const aux = CLIENTES_ALERTAS.some(event);
   ...
   const onOpenEditClienteAlerta = (id) => {
      setClienteAlertaSeleccionado(id);
      setAccion('editar');
      setShowForm(true);
   };
   ...
   const onOpenViewClienteAlerta = (id) => {
      setClienteAlertaSeleccionado(id);
      setAccion('ver');
      setShowForm(true);
   };

   const onDeleteClienteAlerta = (id) => {
      Swal.fire({
         title: 'Confirmar',
         text: '¿ Seguro que desea eliminar la Alerta ?',
   ...
   const onOpenAddClienteAlerta = () => {
      setClienteAlertaSeleccionado(0);
      setAccion('crear');
      setShowForm(true);
   };
   ...
   return (
      <div className = { classes.root }>
         <Paper className = { classes.paper }>
            { permisos && (
               <EnhancedTableToolbar
                  ...
                  titulo = { titulo }
                  onOpenAddClienteAlerta = { onOpenAddClienteAlerta }
                                       ...
                                       return (
                                          <TableRow
                                             ...
                                             <TableCell 
                                                ...
                                                {
                                                   permisos.indexOf('ModificarAle') >= 0 && 
                                                   (
                                                      <Tooltip title = { <IntlMessages id = 'boton.editar' />}>
                                                         <EditIcon
                                                            onClick = { () => onOpenEditClienteAlerta(row.id) }
                                                ...
                                                {
                                                   permisos.indexOf('ListarAle') >= 0 && 
                                                   (
                                                      <Tooltip title = { <IntlMessages id = 'boton.ver' />}>
                                                         <VisibilityIcon
                                                            onClick = { () => onOpenViewClienteAlerta(row.id) }
                                                ...
                                                {
                                                   permisos.indexOf('EliminarAle') >= 0 && 
                                                   (
                                                      <Tooltip
                                                         title = { <IntlMessages id = 'boton.eliminar' />}>
                                                         <DeleteIcon
                                                            onClick = { () => onDeleteClienteAlerta(row.id) }
...

         {
            showForm 
               ?  (
                     <ClienteAlertaCreador
                        showForm = { showForm }
                        clienteAlerta = { clienteAlertaSeleccionado }
                        accion = { accion }
