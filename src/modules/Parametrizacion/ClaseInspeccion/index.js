...
import ClaseInspeccionCreador from './ClaseInspeccionCreador';
import {
  onGetColeccion,
  onDelete,
} from 'redux/actions/ClaseInspeccionAction';
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip
                  title='Crear Clase de Inspeccion'
                  onClick={onOpenAddClaseInspeccion}>
                  <IconButton
...
const ClaseInspeccion = (props) => {
  ...
  const [claseInspeccionSeleccionada, setClaseInspeccionSeleccionada] =
    useState(0);
  ...
  useEffect(() => {
    setPage(1);
  }, [debouncedName, orderByToSend, cliente]);
  ...
  const onOpenEditClaseInspeccion = (id) => {
    setClaseInspeccionSeleccionada(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewClaseInspeccion = (id) => {
    setClaseInspeccionSeleccionada(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteClaseInspeccion = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar La Clase de Inspección?',
  ...
  const onOpenAddClaseInspeccion = () => {
    setClaseInspeccionSeleccionada(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  const onGoListaFotos = (id) => {
    history.push('/lista-fotos/'+id);
  } 
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            onOpenAddClaseInspeccion={onOpenAddClaseInspeccion}
                      ...
                      return (
                        <TableRow
                          ...
                          <TableCell
                            ...
                            {permisos.indexOf('Modificar') >= 0 && (
                              <Tooltip
                                ...
                                <Edit
                                  onClick={() =>
                                    onOpenEditClaseInspeccion(row.id)
                                  }
                            ...
                            {permisos.indexOf('Listar') >= 0 && (
                              <Tooltip title={<IntlMessages id='boton.ver' />}>
                                <Visibility
                                  onClick={() =>
                                    onOpenViewClaseInspeccion(row.id)
                                  }
                            ...
                            {permisos.indexOf('Eliminar') >= 0 && (
                              <Tooltip
                                ...
                                <Delete
                                  onClick={() =>
                                    onDeleteClaseInspeccion(row.id)
                                  }
                          ...
                          <TableCell align='center' className={classes.cell}>
                            <Tooltip title={'Lista Fotos'}>
                              <Box style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                                <AddAPhoto 
                                  style={{
                                    color: '#710382'
                                  }}
                                  onClick={() => onGoListaFotos(row.id)}
      ...
      {showForm ? (
        <ClaseInspeccionCreador
          showForm={showForm}
          claseInspeccion={claseInspeccionSeleccionada}
          accion={accion}
...
export default ClaseInspeccion;
