...
import { onGetColeccionLigera as onGetAsociados } from 'redux/actions/AsociadoAction';
...
import { AssignmentInd, EmojiTransportation, HomeWork, LocalShipping, NotificationImportant, SupervisorAccount } from '@material-ui/icons';
...
const cells = [
  ...
  {
    id: 'indicativo_instalacion_especial',
    typeHead: 'string',
  ...
  {
    id: 'indicativo_lectura_contenedor',
    typeHead: 'string',
    label: 'Lectura Contenedor',
    value: (value) => (value === 'S' ? 'Sí' : 'No'),
    align: 'left',
    mostrarInicio: false,
  },
  {
    id: 'indicativo_contenedor_exclusivo',
    typeHead: 'string',
    label: 'Contenedor Exclusivo',
    value: (value) => (value === 'S' ? 'Sí' : 'No'),
    align: 'left',
    mostrarInicio: false,
  },
...
const Cliente = (props) => {
  ...
  useEffect(() => {
    dispatch(onGetColeccion(page, rowsPerPage, orderByToSend, nombreFiltro));
  }, [dispatch, page, rowsPerPage, debouncedName, orderByToSend, showForm]); //eslint-disable-line
  ...
  const onOpenViewCliente = (id) => {
    setClienteSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteCliente = (id) => {
    Swal.fire({
      title: 'Confirmar',
  ...
  const onGoLugares = (id) => {
    history.push('/lugares/'+id);
  } 
  const onGoAlertas = (id) => {
    history.push('/clientes-alertas/'+id);
  } 
  const onGoVehiculos = (id) => {
    history.push('/clientes-vehiculos/'+id);
  } 
  const onGoConductores = (id) => {
    history.push('/clientes-conductores/'+id);
  } 
  const onGoInspectores = (id) => {
    history.push('/clientes-inspectores/'+id);
  } 
  const onGoEmpresasTransporte = (id) => {
    history.push('/clientes-empresas-transporte/'+id);
  } 
  const getNombreAsociado = (id) => {
    const asociado = asociados.find((asociado) => asociado.id === id);
      ...
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddCliente={onOpenAddCliente}
                            ...
                            if (columna.mostrar) {
                              return (
                                <MyCell
                                  ...
                                  value={
                                    columna.id === 'asociado_id' ?
                                      getNombreAsociado(columna.value(row[columna.id])) :
                              ...
                              <Tooltip title={'Conductores'}>
                                <Box style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                                  <AssignmentInd 
                                    ...
                                    onClick={() => onGoConductores(row.id)}
                                    className={`${classes.generalIcons}`}/>
                                </Box>
                              </Tooltip>
                            </TableCell>
                              ...
                              <Tooltip title={'Inspectores'}>
                                <Box style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                                  <SupervisorAccount 
                                    ...
                                    onClick={() => onGoInspectores(row.id)}
                                    className={`${classes.generalIcons}`}/>
                                </Box>
                              </Tooltip>
                            </TableCell>
                           <TableCell align='center' className={classes.cell}>
                              <Tooltip title={'Empresas Transporte'}>
                                <Box style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                                  <EmojiTransportation
                                    style={{
                                      color: 'blue'
                                    }}
                                    onClick={() => onGoEmpresasTransporte(row.id)}
                                    className={`${classes.generalIcons}`}/>
                                </Box>
                              </Tooltip>
                            </TableCell>
