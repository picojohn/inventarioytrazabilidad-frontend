...
import LugarUsuarioCreador from './LugarUsuarioCreador';
import {
  onGetColeccion,
  onDelete,
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip
                  title='Crear Lugar de Usuario'
                  onClick={onOpenAddLugarUsuario}>
                  <IconButton
...
const LugarUsuario = (props) => {
  ...
  const [lugarUsuarioSeleccionado, setLugarUsuarioSeleccionado] =
    useState(0);
  ...
  const debouncedPlace = useDebounce(lugar, 800);
  ...
  }, [dispatch, page, rowsPerPage, debouncedName, debouncedPlace, cliente, orderByToSend]); //eslint-disable-line

  useEffect(() => {
    setPage(1);
  }, [debouncedName, debouncedPlace, cliente, orderByToSend]);
  ...
  const onOpenEditLugarUsuario = (id) => {
    setLugarUsuarioSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewLugarUsuario = (id) => {
    setLugarUsuarioSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteLugarUsuario = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar El Lugar de Usuario?',
  ...
  const onOpenAddLugarUsuario = () => {
    setLugarUsuarioSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddLugarUsuario={onOpenAddLugarUsuario}
      ...
      {showForm ? (
        <LugarUsuarioCreador
          showForm={showForm}
          lugarUsuario={lugarUsuarioSeleccionado}
          accion={accion}
