...
import FormatoInspeccionCreador from './FormatoInspeccionCreador';
import {
  onGetColeccion,
  onDelete,
} from 'redux/actions/FormatoInspeccionAction';
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip
                  title='Crear Formato de Inspección'
                  onClick={onOpenAddFormatoInspeccion}>
                  <IconButton
...
const FormatoInspeccion = (props) => {
  ...
  const [formatoInspeccionSeleccionado, setFormatoInspeccionSeleccionado] =
    useState(0);
  ...
  useEffect(() => {
    if (message && message === 'El formato de inspeccion ha sido eliminado.') {
  ...
  }, [dispatch, page, rowsPerPage, debouncedName, cliente, orderByToSend]); //eslint-disable-line
  ...
  const onOpenEditFormatoInspeccion = (id) => {
    setFormatoInspeccionSeleccionado(id);
    setAccion('editar');
    setShowForm({
      ...initialShow,
      creador: true
  ...
  const onOpenViewFormatoInspeccion = (id) => {
    setFormatoInspeccionSeleccionado(id);
    setAccion('ver');
    setShowForm({
      ...initialShow,
  ...
  const onDeleteFormatoInspeccion = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar El Formato de Inspección?',
  ...
  const onOpenAddFormatoInspeccion = () => {
    setFormatoInspeccionSeleccionado(0);
    setAccion('crear');
    setShowForm({
      ...initialShow,
      creador: true
  ...
  const onOpenUnidades = (row) => {
    setFormatoInspeccionSeleccionado(row);
    setShowForm({
      ...initialShow,
      unidades: true
    })
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            onOpenAddFormatoInspeccion={onOpenAddFormatoInspeccion}
                          ...
                          <TableCell
                            ...
                            {permisos.indexOf('Modificar') >= 0 && (
                              <Tooltip
                                ...
                                <Edit
                                  onClick={() =>
                                    onOpenEditFormatoInspeccion(row.id)
                                  }
                            ...
                            {permisos.indexOf('Listar') >= 0 && (
                              <Tooltip title={<IntlMessages id='boton.ver' />}>
                                <Visibility
                                  onClick={() =>
                                    onOpenViewFormatoInspeccion(row.id)
                                  }
                            ...
                            {permisos.indexOf('Eliminar') >= 0 && (
                              <Tooltip
                                ...
                                <Delete
                                  onClick={() =>
                                    onDeleteFormatoInspeccion(row.id)
                                  }
      ...
      {showForm.creador && (
        <FormatoInspeccionCreador
          showForm={showForm.creador}
          formatoInspeccion={formatoInspeccionSeleccionado}
          accion={accion}
...
export default FormatoInspeccion;
