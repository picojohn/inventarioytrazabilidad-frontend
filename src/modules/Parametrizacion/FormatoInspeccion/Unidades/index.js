...
import UnidadCreador from './UnidadCreador';
...
const cells = [
  ...
  },
  {
    ...
    typeHead: 'string',
    label: 'Tipo Unidad',
    value: (value) => UNIDADES_CARGA_TRANSPORTE.map((und) => und.id === value ? und.nombre : ''),
    align: 'left',
    mostrarInicio: true,
  },
  {
        ...
        <>
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              <Tooltip
                ...
                onClick={onOpenAddUnidadFormato}>
                <IconButton
          ...
          <Box className={classes.contenedorFiltros}>
            <TextField
              ...
              name='formatoInspeccion'
              id='formatoInspeccion'
              disabled
              value={formatoInspeccion?.nombre??''}
...
const Unidades = (props) => {
  ...
  const [unidadFormatoSeleccionado, setUnidadFormatoSeleccionado] =
    useState(0);
  ...
  useEffect(() => {
    if(formatoInspeccion){
      dispatch(onGetColeccion(page, rowsPerPage, orderByToSend, nombre, formatoInspeccion.id));
    }
  }, [page, rowsPerPage, debouncedName, orderByToSend, formatoInspeccion]); //eslint-disable-line
  ...
  const onOpenEditUnidadFormato = (row) => {
    setUnidadFormatoSeleccionado(row);
    setAccion('editar');
    setShowForm({
      ...initialShow,
      creador: true
  ...
  const onDeleteUnidadFormato = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar La Unidad del Formato?',
  ...
  const onOpenAddUnidadFormato = () => {
    setUnidadFormatoSeleccionado(0);
    setAccion('crear');
    setShowForm({
      ...initialShow,
      creador: true
  ...
  const renderListas = (list) => {
    return list.map((it) => it.nombre).join(', ');
  }
        ...
        <div className={classes.root}>
          <Paper className={classes.paper}>
            <EnhancedTableToolbar
              onOpenAddUnidadFormato={onOpenAddUnidadFormato}
                                ...
                                if (columna.mostrar) {
                                  return (
                                    <MyCell
                                      ...
                                      value={columna.id === 'listas' ? renderListas(row[columna.id]) : columna.value(row[columna.id])}
                                      cellColor={
                                        columna.cellColor
          ...
          {showForm.creador && (
            <UnidadCreador
              ...
              unidadFormato={unidadFormatoSeleccionado}
              formatoInspeccion={formatoInspeccion}
              accion={accion}
