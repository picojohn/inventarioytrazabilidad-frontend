...
import ClienteConductorCreador from './ClienteConductorCreador';
import {
   onGetHeaders,
   onGetColeccion, 
   onDelete,
...
import { CLIENTES_CONDUCTORES } from 'shared/constants/SubmenuOptions';
...
const cells = [
  ...
  {
    id: 'indicativo_conductor_empresa',
    typeHead: 'string',
               ...
               <>
                  <Box className = { classes.titleTop }>
                     ...
                     <Box className = { classes.horizontalBottoms }>
                        ...
                        {permisos.indexOf('CrearCon') >= 0 && (
                           <Tooltip title = 'Crear Conductor' onClick = { onOpenAddClienteConductor }>
                              <IconButton
...
EnhancedTableToolbar.propTypes = {
   numSelected: PropTypes.number.isRequired,
   onOpenAddClienteConductor: PropTypes.func.isRequired,
...
const ClienteConductors = (props) => {
   ...
   const [clienteConductorSeleccionado, setClienteConductorSeleccionado] = useState(0);
   const { rows, desde, hasta, ultima_pagina, total } = useSelector(
      ({ clienteConductorReducer }) => clienteConductorReducer,
   );

   const { message, error, messageType } = useSelector(({ common }) => common);
   const headers = useSelector(({ clienteConductorReducer }) => clienteConductorReducer.ligera);
         ...
         user.permisos.forEach((modulo) => {
            modulo.opciones.forEach((opcion) => {
               if (opcion.url === '/clientes') {
                  setTitulo(`${opcion.nombre} - Conductores`);
                  const permisoAux = [];
                  opcion.permisos.forEach((permiso) => {
                     const event = (perm) => perm.nombre === permiso.titulo;
                     const aux = CLIENTES_CONDUCTORES.some(event);
   ...
   const onOpenEditClienteConductor = (id) => {
      setClienteConductorSeleccionado(id);
      setAccion('editar');
      setShowForm(true);
   };
   ...
   const onOpenViewClienteConductor = (id) => {
      setClienteConductorSeleccionado(id);
      setAccion('ver');
      setShowForm(true);
   };

   const onDeleteClienteConductor = (id) => {
      Swal.fire({
         title: 'Confirmar',
         text: '¿ Seguro que desea eliminar el Conductor ?',
   ...
   const onOpenAddClienteConductor = () => {
      setClienteConductorSeleccionado(0);
      setAccion('crear');
      setShowForm(true);
   };
   ...
   return (
      <div className = { classes.root }>
         <Paper className = { classes.paper }>
            { permisos && (
               <EnhancedTableToolbar
                  ...
                  titulo = { titulo }
                  onOpenAddClienteConductor = { onOpenAddClienteConductor }
                                       ...
                                       return (
                                          <TableRow
                                             ...
                                             <TableCell 
                                                ...
                                                {
                                                   permisos.indexOf('ModificarCon') >= 0 && 
                                                   (
                                                      <Tooltip title = { <IntlMessages id = 'boton.editar' />}>
                                                         <EditIcon
                                                            onClick = { () => onOpenEditClienteConductor(row.id) }
                                                ...
                                                {
                                                   permisos.indexOf('ListarCon') >= 0 && 
                                                   (
                                                      <Tooltip title = { <IntlMessages id = 'boton.ver' />}>
                                                         <VisibilityIcon
                                                            onClick = { () => onOpenViewClienteConductor(row.id) }
                                                ...
                                                {
                                                   permisos.indexOf('EliminarCon') >= 0 && 
                                                   (
                                                      <Tooltip
                                                         title = { <IntlMessages id = 'boton.eliminar' />}>
                                                         <DeleteIcon
                                                            onClick = { () => onDeleteClienteConductor(row.id) }
...

         {
            showForm 
               ?  (
                     <ClienteConductorCreador
                        showForm = { showForm }
                        clienteConductor = { clienteConductorSeleccionado }
                        accion = { accion }
...
export default ClienteConductors;
