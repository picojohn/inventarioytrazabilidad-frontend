...
import ProductoClienteCreador from './ProductoClienteCreador';
import {
  onGetColeccion,
  onDelete,
} from '../../../redux/actions/ProductoClienteAction';
import { onGetColeccionLigera as onGetProductos } from 'redux/actions/ProductoAction';
...
const cells = [
  ...
  {
    id: 'indicativo_producto_externo',
    typeHead: 'string',
    label: 'Producto Externo',
    value: (value) => (value === 'S' ? 'Sí' : 'No'),
    align: 'left',
    mostrarInicio: false,
    cellColor: (value) => (value === 'S' ? 'green' : 'red'),
  },
  {
    id: 'valor_serial_interno',
    typeHead: 'numeric',
  ...
  {
    id: 'operador_serial_interno',
    typeHead: 'string',
    label: 'Op. Serial Interno',
    value: (value) => value,
    align: 'left',
    mostrarInicio: false,
  },
  ...
  {
    id: 'valor_serial_datamatrix',
    typeHead: 'numeric',
    label: 'Serial Datamatrix',
    value: (value) => value,
    align: 'right',
    mostrarInicio: false,
  },
  {
    id: 'operador_serial_datamatrix',
    typeHead: 'string',
    label: 'Op. Serial Datamatrix',
    value: (value) => value,
    align: 'left',
    mostrarInicio: false,
  },
  ...
  {
    id: 'indicativo_producto_empaque',
    typeHead: 'string',
    label: 'Empaque',
    value: (value) => (value === 'S' ? 'Sí' : 'No'),
    align: 'left',
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip
                  ...
                  onClick={onOpenAddProductoCliente}>
                  <IconButton
...
const ProductoCliente = (props) => {
  ...
  const [productoClienteSeleccionado, setProductoClienteSeleccionado] =
    useState(0);
  ...
  useEffect(() => {
    dispatch(onGetColeccion(page, rowsPerPage, orderByToSend, nombreFiltro, clienteFiltro));
  }, [dispatch, page, rowsPerPage, debouncedName, orderByToSend, showForm, clienteFiltro]); //eslint-disable-line
  ...
  useEffect(() => {
    setPage(1);
  }, [debouncedName, orderByToSend, clienteFiltro]);
  ...
  const onOpenEditProductoCliente = (id) => {
    setProductoClienteSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewProductoCliente = (id) => {
    setProductoClienteSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteProductoCliente = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar El Producto del Cliente?',
  ...
  const onOpenAddProductoCliente = () => {
    setProductoClienteSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            ...
            onOpenAddProductoCliente={onOpenAddProductoCliente}
                          ...
                          <TableCell
                            ...
                            {permisos.indexOf('Modificar') >= 0 && (
                              <Tooltip
                                ...
                                <EditIcon
                                  onClick={() =>
                                    onOpenEditProductoCliente(row.id)
                                  }
                            ...
                            {permisos.indexOf('Listar') >= 0 && (
                              <Tooltip title={<IntlMessages id='boton.ver' />}>
                                <VisibilityIcon
                                  onClick={() =>
                                    onOpenViewProductoCliente(row.id)
                                  }
                            ...
                            {permisos.indexOf('Eliminar') >= 0 && (
                              <Tooltip
                                ...
                                <DeleteIcon
                                  onClick={() =>
                                    onDeleteProductoCliente(row.id)
                                  }
                                ...
                                <MyCell
                                  ...
                                  value={
                                    columna.id === 'producto_s3_id' ?
                                      getNombreProducto(columna.value(row[columna.id])) :
      ...
      {showForm ? (
        <ProductoClienteCreador
          showForm={showForm}
          productoCliente={productoClienteSeleccionado}
          accion={accion}
