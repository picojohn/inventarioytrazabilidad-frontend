...
import InventarioMinimoCreador from './InventarioMinimoCreador';
import {
  onGetColeccion,
  onDelete,
      ...
      <>
        <Box className={classes.titleTop}>
          ...
          <Box className={classes.horizontalBottoms}>
            ...
            {permisos.indexOf('Crear') >= 0 && (
              <Tooltip
                title='Crear Inventario Mínimo'
                onClick={onOpenAddInventarioMinimo}>
                <IconButton
        ...
        <Box className={classes.contenedorFiltros}>
          ...
          <TextField
              ...
              className={classes.inputFiltros}
            >
              {lugares.filter((place) => cliente ? (place.cliente_id === parseInt(cliente)) : place.cliente_id === user?.asociado?.id).map((place) => {
                return (
                  <MenuItem
                    value={place.id}
                    key={place.id}
...
const InventarioMinimo = (props) => {
  ...
  const [inventarioMinimoSeleccionado, setInventarioMinimoSeleccionado] =
    useState(0);
  ...
  useEffect(() => {
    dispatch(onGetColeccion(page, rowsPerPage, orderByToSend, nombre, lugar, cliente));
  }, [dispatch, page, rowsPerPage, debouncedName, lugar, cliente, orderByToSend]); //eslint-disable-line

  useEffect(() => {
    setPage(1);
  }, [debouncedName, lugar, cliente, orderByToSend]);
  ...
  const onOpenEditInventarioMinimo = (id) => {
    setInventarioMinimoSeleccionado(id);
    setAccion('editar');
    setShowForm(true);
  };
  ...
  const onOpenViewInventarioMinimo = (id) => {
    setInventarioMinimoSeleccionado(id);
    setAccion('ver');
    setShowForm(true);
  };

  const onDeleteInventarioMinimo = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar El Registro de Inventario Mínimo?',
  ...
  const onOpenAddInventarioMinimo = () => {
    setInventarioMinimoSeleccionado(0);
    setAccion('crear');
    setShowForm(true);
  };
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            onOpenAddInventarioMinimo={onOpenAddInventarioMinimo}
                          ...
                          <TableCell
                            ...
                            {permisos.indexOf('Modificar') >= 0 && (
                              <Tooltip
                                ...
                                <EditIcon
                                  onClick={() =>
                                    onOpenEditInventarioMinimo(row.id)
                                  }
                            ...
                            {permisos.indexOf('Listar') >= 0 && (
                              <Tooltip title={<IntlMessages id='boton.ver' />}>
                                <VisibilityIcon
                                  onClick={() =>
                                    onOpenViewInventarioMinimo(row.id)
                                  }
                            ...
                            {permisos.indexOf('Eliminar') >= 0 && (
                              <Tooltip
                                ...
                                <DeleteIcon
                                  onClick={() =>
                                    onDeleteInventarioMinimo(row.id)
                                  }
      ...
      {showForm && (
        <InventarioMinimoCreador
          showForm={showForm}
          inventarioMinimo={inventarioMinimoSeleccionado}
          accion={accion}
...
export default InventarioMinimo;
