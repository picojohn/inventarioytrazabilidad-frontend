...
import ClienteVehiculoCreador from './ClienteVehiculoCreador';
import {
   onGetHeaders,
   onGetColeccion, 
   onDelete,
...
import { CLIENTES_VEHICULOS } from 'shared/constants/SubmenuOptions';
               ...
               <>
                  <Box className = { classes.titleTop }>
                     ...
                     <Box className = { classes.horizontalBottoms }>
                        ...
                        {permisos.indexOf('CrearVeh') >= 0 && (
                           <Tooltip title = 'Crear Vehículo' onClick = { onOpenAddClienteVehiculo }>
                              <IconButton
...
EnhancedTableToolbar.propTypes = {
   numSelected: PropTypes.number.isRequired,
   onOpenAddClienteVehiculo: PropTypes.func.isRequired,
...
const ClienteVehiculos = (props) => {
   ...
   const [clienteVehiculoSeleccionado, setClienteVehiculoSeleccionado] = useState(0);
   const { rows, desde, hasta, ultima_pagina, total } = useSelector(
      ({ clienteVehiculoReducer }) => clienteVehiculoReducer,
   );

   const { message, error, messageType } = useSelector(({ common }) => common);
   const headers = useSelector(({ clienteVehiculoReducer }) => clienteVehiculoReducer.ligera);
         ...
         user.permisos.forEach((modulo) => {
            modulo.opciones.forEach((opcion) => {
               if (opcion.url === '/clientes') {
                  setTitulo(`${opcion.nombre} - Vehículos`);
                  const permisoAux = [];
                  opcion.permisos.forEach((permiso) => {
                     const event = (perm) => perm.nombre === permiso.titulo;
                     const aux = CLIENTES_VEHICULOS.some(event);
   ...
   const onOpenEditClienteVehiculo = (id) => {
      setClienteVehiculoSeleccionado(id);
      setAccion('editar');
      setShowForm(true);
   };
   ...
   const onOpenViewClienteVehiculo = (id) => {
      setClienteVehiculoSeleccionado(id);
      setAccion('ver');
      setShowForm(true);
   };

   const onDeleteClienteVehiculo = (id) => {
      Swal.fire({
         title: 'Confirmar',
         text: '¿ Seguro que desea eliminar el Vehículo ?',
   ...
   const onOpenAddClienteVehiculo = () => {
      setClienteVehiculoSeleccionado(0);
      setAccion('crear');
      setShowForm(true);
   };
   ...
   return (
      <div className = { classes.root }>
         <Paper className = { classes.paper }>
            { permisos && (
               <EnhancedTableToolbar
                  ...
                  titulo = { titulo }
                  onOpenAddClienteVehiculo = { onOpenAddClienteVehiculo }
                                       ...
                                       return (
                                          <TableRow
                                             ...
                                             <TableCell 
                                                ...
                                                {
                                                   permisos.indexOf('ModificarVeh') >= 0 && 
                                                   (
                                                      <Tooltip title = { <IntlMessages id = 'boton.editar' />}>
                                                         <EditIcon
                                                            onClick = { () => onOpenEditClienteVehiculo(row.id) }
                                                ...
                                                {
                                                   permisos.indexOf('ListarVeh') >= 0 && 
                                                   (
                                                      <Tooltip title = { <IntlMessages id = 'boton.ver' />}>
                                                         <VisibilityIcon
                                                            onClick = { () => onOpenViewClienteVehiculo(row.id) }
                                                ...
                                                {
                                                   permisos.indexOf('EliminarVeh') >= 0 && 
                                                   (
                                                      <Tooltip
                                                         title = { <IntlMessages id = 'boton.eliminar' />}>
                                                         <DeleteIcon
                                                            onClick = { () => onDeleteClienteVehiculo(row.id) }
...

         {
            showForm 
               ?  (
                     <ClienteVehiculoCreador
                        showForm = { showForm }
                        clienteVehiculo = { clienteVehiculoSeleccionado }
                        accion = { accion }
