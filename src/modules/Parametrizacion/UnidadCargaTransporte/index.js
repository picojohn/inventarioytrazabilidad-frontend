...
import UnidadCargaTransporteCreador from './UnidadCargaTransporteCreador';
...
import { UNIDADES_CARGA_TRANSPORTE } from 'shared/constants/ListaValores';
import { LISTAS_CHEQUEO_UNIDADES, TIPOS_CHEQUEO_LISTAS_UNIDADES } from 'shared/constants/SubmenuOptions';
...
const cells = [
  ...
  {
    id: 'indicativo_tipo_unidad',
    typeHead: 'string',
    label: 'Tipo',
    value: (value) => UNIDADES_CARGA_TRANSPORTE.map((unidad) => unidad.id === value ? unidad.nombre : ''),
    align: 'left',
    mostrarInicio: true,
  },
  {
...
function EnhancedTableHead(props) {
  const {classes, order, orderBy, onRequestSort, columnasMostradas, cLPermisos} = props;
...
const EnhancedTableToolbar = (props) => {
  ...
  const {
    titulo,
    onOpenAddUnidadCargaTransporte,
          ...
          <Box className={classes.titleTop}>
            ...
            <Box className={classes.horizontalBottoms}>
              ...
              {permisos.indexOf('Crear') >= 0 && (
                <Tooltip
                  title='Crear Unidad de Carga o Transporte'
                  onClick={onOpenAddUnidadCargaTransporte}>
                  <IconButton
...
const checkListPermissions = (permisos, permissions) => {
  let cLPermisos = [];
  if(permisos.length > 0){
    cLPermisos = permisos.reduce((result, permiso) => {
      const event = (perm) => perm.nombre === permiso;
      if(permissions.some(event)){
        const index = permissions.findIndex((perm) => perm.nombre === permiso);
        result.push(permissions[index].accion);  
      }
      return result;
    },[]); 
  }
...
const UnidadCargaTransporte = (props) => {
  ...
  const [unidadCargaTransporteSeleccionado, setUnidadCargaTransporteSeleccionado] =
    useState(0);
  ...
  useEffect(() => {
    if (message && message === 'La unidad de carga o transporte ha sido eliminada.') {
  ...
  }, [page, rowsPerPage, debouncedName, cliente, orderByToSend]); //eslint-disable-line
  ...
  const onOpenEditUnidadCargaTransporte = (id) => {
    setUnidadCargaTransporteSeleccionado(id);
    setAccion('editar');
    setShowForm({
      ...initialFormStatus,
      unidad: true
  ...
  const onDeleteUnidadCargaTransporte = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Eliminar La Unidad de Carga/Transporte?',
  ...
  const onOpenAddUnidadCargaTransporte = () => {
    setUnidadCargaTransporteSeleccionado(0);
    setAccion('crear');
    setShowForm({
      ...initialFormStatus,
      unidad: true
  ...
  const openListaChequeo = (row) => {
    setUnidadCargaTransporteSeleccionado(row)
    setShowForm({
      ...initialFormStatus,
      listaChequeo: true
    });
  ...
  const cLPermisos = useMemo(() => checkListPermissions(permisos, LISTAS_CHEQUEO_UNIDADES), [permisos]);
  const tCLUPermisos = useMemo(() => checkListPermissions(permisos, TIPOS_CHEQUEO_LISTAS_UNIDADES), [permisos]);
  ...
  return (
    <div className={classes.root}>
      <Paper className={classes.paper}>
        {permisos && (
          <EnhancedTableToolbar
            onOpenAddUnidadCargaTransporte={onOpenAddUnidadCargaTransporte}
                        ...
                        <TableRow
                          ...
                          <TableCell
                            ...
                            {permisos.indexOf('Modificar') >= 0 && (
                              <Tooltip
                                ...
                                <Edit
                                  onClick={() =>
                                    onOpenEditUnidadCargaTransporte(row.id)
                                  }
                            ...
                            {permisos.indexOf('Eliminar') >= 0 && (
                              <Tooltip
                                ...
                                <Delete
                                  onClick={() =>
                                    onDeleteUnidadCargaTransporte(row.id)
                                  }
                          ...
                          { cLPermisos.length > 0 && (
                            <TableCell align='center' className={classes.cell}>
                              <Tooltip title={'Listas de Chequeo'}>
                                <Box style={{display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                                  <List 
                                    onClick={() => openListaChequeo(row)}
      ...
      {showForm.unidad && (
        <UnidadCargaTransporteCreador
          showForm={showForm.unidad}
          unidadCargaTransporte={unidadCargaTransporteSeleccionado}
          accion={accion}
      ...
      { showForm.listaChequeo && (
        <ListaChequeo
          showForm={showForm.listaChequeo}
          unidadCargaTransporte={unidadCargaTransporteSeleccionado}
          accion={accion}
          handleOnClose={handleOnClose}
          updateColeccion={updateColeccion}
          titulo={'Tipos Listas Chequeo'}
          permisosL={cLPermisos}
          permisosT={tCLUPermisos}
        />
      )}
