...
import PedidoDetalleEditor from '../LecturaKit';
...
const Leidos = (props) => {
  const {pedido, protegido, leido, productosS3} = props;
  ...
  useEffect(() => {
    if (message) {
      if (messageType === DELETE_TYPE) {
        Swal.fire('Devuelto para lectura', message, 'success');
        updateColeccion();
      }
    }
  ...
  const debouncedNumeroPedido = useDebounce(numeroPedido, 700);
  ...
  useEffect(() => {
    if(pedido?.numero_pedido){
      dispatch(onGetColeccion(page, rowsPerPage, orderByToSend, pedido.numero_pedido, numeroPedido));
    }
  }, [dispatch, page, rowsPerPage, orderByToSend, pedido, debouncedNumeroPedido]); //eslint-disable-line
  ...
  const onOpenEditDetalleKit = (id) => {
    Swal.fire({
      title: 'Confirmar',
      text: '¿Seguro Que Desea Devolver el Sello/Kit Para Lectura?',
